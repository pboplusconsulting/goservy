...
<div class="row">
		...
		foreach($tableStatus as $ts){
            ...
            {
				$no_of_orders[$i++]=$this->Chef_model->total_orders($booking_id);
            ...
            {
				$complete_orders[$j++]=$this->Chef_model->no_of_complete_order($booking_id);
			}	
			else
			{
				$complete_orders[$j++]=0;
            ...
            {
				$cancel_orders[$c++]=$this->Chef_model->no_of_cancel_order($booking_id);
			}	
			else
			{
				$cancel_orders[$c++]=0;
	  ...
	  <?php } ?>
        ...
        <div class="col-md-4 tab_mob_activated col-sm-12 col-xs-12">
        	<section id="members_listing">
            	<h4 class="chef_screen">Chef - Today's Member</h4>
            ...
            <div id="start_lsiting">
				...
				<?php $in=0; foreach($data as $member){
			        $orderData=$this->Chef_model->get_order_data($member['table_booking_id']);
					
					$no_of_delay=$this->Chef_model->no_of_dalay_order($member['table_booking_id']);
					$no_of_complete=$this->Chef_model->no_of_complete_order($member['table_booking_id']);
			        $no_of_cancel=$this->Chef_model->no_of_cancel_order($member['table_booking_id']);
					$no_of_unread=$this->Chef_model->no_of_unread_oreder($member['table_booking_id']);
                    $total_orders=$this->Chef_model->total_orders($member['table_booking_id']);
					//last_order_time=$this->Chef_model->get_last_order_time($member['table_booking_id']);
					//print_r($total_orders);
					//echo $last_order_time[0]->order_time;
					
					$feedback=$this->Chef_model->get_by_query_return_row("SELECT * FROM ru_feedback WHERE table_booking_id=".$member['table_booking_id']." AND status=1");
