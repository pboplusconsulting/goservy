...
class Dashboard extends CI_Controller {
	...
	{   //print_r($_SESSION);die();
		...
		if(isset($_POST['add_new_member']))
		{
			// print_r($_POST);;
				...
				$random_member_id = random_string('nozero', 8);			
				$memberData=array(
				                'membership_id'=>$random_member_id,
				...
				$insertId=$this->Dash_model->insert_one_row('ru_membership',$memberData);
				$recreated_id = $this->db->insert_id();
                if($insertId)
	...
	public function search_member()
	{
		...
		{
			$limit=10;
			$query="SELECT a.*,(SELECT COUNT(*) FROM ru_member_table_booking b WHERE b.member_id=a.id AND b.status=1 AND b.table_status=1) AS booking_status,c.table_booking_id,GROUP_CONCAT(r.table_name) AS table_name,GROUP_CONCAT(r.id) AS table_ids ,c.no_of_guest,c.booking_time,c.table_status,c.member_status,c.remark,c.cancel_reason FROM ru_membership a LEFT JOIN ru_member_table_booking c ON c.member_id=a.id AND c.status=1 AND c.table_status=1 LEFT JOIN ru_tables r ON FIND_IN_SET(r.id,c.table_booking_no) WHERE a.membership_id LIKE '%".$search_string."%' OR a.member_name LIKE '%".$search_string."%' OR a.phone_number LIKE '%".$search_string."%' AND a.status=1 GROUP BY a.id ORDER BY a.member_name";
			// print($query);
	...
	public function generate_bill()
	{
        ...
        else if(isset($_POST['action']) && $_POST['action']=='save_print')
        {
			...
			$final_amount=$total_amount+$totalTax;
			$saveBillData=array(
