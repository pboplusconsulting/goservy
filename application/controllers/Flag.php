...
class Flag extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($search_str))
		{
			$query='SELECT a.*,b.category_name FROM ru_taxes a JOIN ru_product_category b ON a.product_category_id=b.product_category_id WHERE a.product_category_id IN (1,2) AND a.tax_name LIKE "%'.$search_str.'%"';
			$query1='SELECT a.*,b.category_name FROM ru_taxes a JOIN ru_product_category b ON a.product_category_id=b.product_category_id WHERE a.product_category_id IN (1,2) AND a.tax_name LIKE "%'.$search_str.'%" ORDER BY tax_name ASC LIMIT '.$limit.' OFFSET '.$offset;
		}	
		else
		{	
			$query='SELECT a.*,b.category_name FROM ru_taxes a JOIN ru_product_category b ON a.product_category_id=b.product_category_id WHERE a.product_category_id IN (1,2)';
			$query1='SELECT a.*,b.category_name FROM ru_taxes a JOIN ru_product_category b ON a.product_category_id=b.product_category_id WHERE a.product_category_id IN (1,2) ORDER BY a.tax_name ASC LIMIT '.$limit.' OFFSET '.$offset;
		}	
		//echo $query;die();
