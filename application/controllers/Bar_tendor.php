...
class Bar_tendor extends CI_Controller {
	...
	function __construct()
    {
	    ...
	    $this->load->model('Bar_tendor_model');
	...
	public function index()
	{
		if(isset($_POST['complete_order_status']))
		{
            ...
            {
				$notificationToUsers=$this->Bar_tendor_model->get_by_query("SELECT user_id FROM ru_users WHERE role!=4 AND status=1");
				$table_booking_id='';
				foreach($completeOrders as $order_id)
				{
					...
					$orderData=$this->Bar_tendor_model->get_by_query_return_row($query);
			        $updateStatus=$this->Bar_tendor_model->update_by_condition('ru_table_wise_order',array('order_status'=>1,'completed_time'=>date('Y-m-d H:i:s'),'modify_time'=>date('Y-m-d H:i:s')),array('table_order_id'=>$order_id));
					if($updateStatus)
					{
						...
						$this->Bar_tendor_model->insert_one_row('ru_notifications',$notificationData1);
						...
						foreach($notificationToUsers as $notificationToUser)
						{
							...
							$insertid=$this->Bar_tendor_model->insert_one_row('ru_notifications',$notificationData);
						}	
				...
				{	
				    $pendingOrders=$this->Bar_tendor_model->no_of_pending_order($table_booking_id);
					if($pendingOrders==0)
					{	
				         $updateTableStatus=$this->Bar_tendor_model->update_by_condition('ru_member_table_booking',array('member_status'=>3,'modify_time'=>date('Y-m-d H:i:s')),array('table_booking_id'=>$table_booking_id));
					}
				}	
        ...
        {			
			...
			$result=$this->Bar_tendor_model->get_by_query($query);
			...
			$tableStatus=$this->Bar_tendor_model->get_by_query($query2);
			//print_r($tableStatus);die();
			$heading['heading']="Bar Tendor Dashboard";
			$dashboardData['data']=$result;
			$dashboardData['tableStatus']=$tableStatus;
			$this->load->view('header',$heading);
		    $this->load->view('bar_tendor/bar_tendor_dashboard',$dashboardData);
	...
	public function search_member()
	{
		...
		if(!empty($search_string))
		{
			...
			$result=$this->Bar_tendor_model->get_by_query($query);//echo "<pre>";print_r($result);echo "</pre>";die();
			//echo "<pre>";print_r($result);echo "</pre>";die();
			//$availableTables=$this->Bar_tendor_model->leftjoin_orderby_and_result_array('ru_tables a','a.status=1 AND b.booking_timeb.table_status=0 ','a.id,a.table_name','ru_table_wise_booking b','a.id=b.table_booking_no','table_name','ASC')
			...
			//$availableTables=$this->Bar_tendor_model->get_by_query($sql);
        ...
        else
        {
			...
			$result=$this->Bar_tendor_model->get_by_query($query);//echo "<pre>";print_r($result);echo "</pre>";
			$searchData['data']=$result;
			//$searchData['availableTables']=$availableTables;
			$html=$this->load->view('chef/search_html',$searchData,true);
			echo $html;
	...
	public function update_order_status()
	{
		...
		$resultAffected=$this->Bar_tendor_model->update_by_condition('ru_table_wise_order',array('is_read'=>1),array('table_booking_id'=>$table_booking_id));
        if($resultAffected>0)
		   echo 1;
	    else
			echo 0;
	...
	public function order_cancel(){
		...
		{	
			...
			$resultAffected=$this->Bar_tendor_model->update_by_condition('ru_table_wise_order',array('order_status'=>3,'cancel_reason'=>$comment,'cancel_time'=>date('Y-m-d H:i:s')),array('table_order_id'=>$table_order_id));
