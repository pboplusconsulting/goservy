	...
	public function make_order()
	{
        ...
        { // echo "<pre>";print_r($_POST);echo "</pre>";die();
			...
			if(isset($_POST['meal']) && isset($_POST['quantity']))
			{
				...
				$print_order_id = random_string('numeric', 8);
			...
			// print_r($orderData);
			$print_member_query="SELECT a.*,b.member_name,b.membership_id,c.table_name FROM ru_member_table_booking a JOIN ru_membership b ON a.member_id=b.id LEFT JOIN ru_tables c ON a.table_booking_no=c.id WHERE a.table_booking_id=".$table_booking_id;
				$printMemberData=$this->Menu_model->get_row_by_query($print_member_query);
				
			$get_print_order="SELECT a.*,b.meal_name FROM ru_table_wise_order a JOIN ru_meal b ON a.meal_id=b.meal_id WHERE a.print_order_id=".$print_order_id;
			$printData=$this->Menu_model->get_result_by_query($get_print_order);			
			$orderHtml='';
			$orderHtmlData['orderid']=$print_order_id;
			$orderHtmlData['printremark']=$remark;
			$orderHtmlData['printOrder']=$printData;
			$orderHtmlData['memberData']=$printMemberData;			
			$orderHtml=$this->load->view('menu/bill_pdf_new',$orderHtmlData,true);
		    ...
		    $this->pdf1->load_view('menu/bill_pdf_new',$orderHtmlData,true);
		    $this->pdf1->render();		
			/*******************************************end****************************************/
            ...
            //$this->pdf->set_option('dpi', 72);
		    $this->pdf2->load_view('menu/bill_pdf_new',$orderHtmlData,true);
		    $this->pdf2->render();
			/*******************************************end****************************************/
			
		    $file_name="order-".$print_order_id.".pdf";

            $pdf=$this->pdf2->output();
            if(file_put_contents('uploads/pdf/new_order/'.$file_name,$pdf))
            {
				$saveBillData1['bill_pdf']='uploads/pdf/new_order/'.$file_name;
            }
			
			$send_order_for_print = base_url().'uploads/pdf/new_order/'.$file_name;
			$send_to_dashboard = base_url().'dashboard';
			
			echo "<script language=\"javascript\">
			location.href='".$send_to_dashboard."';
					var w=window.open('".$send_order_for_print."');
					   w.print();
			</script>"; 
