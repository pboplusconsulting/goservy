$(document).ready(function(){
	...
	$('body').on('change',':input.discount',function(e){
		...
		else
		{
			...
			$('#'+parentTableId+' td.total_bill').html(tatalAmount);
			...
			$('#'+parentTableId+' td.grossamount').html((tatalAmount+totalTax).toFixed(2));	
			//window.print();
		}
	});
	$('body').on('submit','form[class="print_order"]',function(e){
        e.preventDefault();
		// $('input[name="meal[]"]').each(function() { var aValue = $(this).val(); alert(aValue); });
...
alert(data_to_send);
       		...
       		$.ajax({
				...
				url:base_url+'menu/make_order',
				type:'post',
				//data:{order:'save_order'},
				  data: data_to_send, 
	...
	$('body').on('submit','form[class="save_print_bill"]',function(e){
		...
		{	
			$.ajax({
				...
				type:'post',
				data:{table_booking_id:table_booking_id,discount:discount,comment:comment,action:'save_print',bill_category:'meal'},
				success:function(result){//alert(result);
				location.reload();
					var w=window.open(base_url+result);
					$(w).on('load',function(){
